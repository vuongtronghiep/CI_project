...
<div class="main1">
	<form action="<?php echo ARSENAL_BASE_URL;?>admin/qlthongke/thongke" method="post">

		Ngày :&nbsp; &nbsp;<input type="date" name="a" value="<?php echo $ngay?>"><br><br>
		...
		Tổng tiền trên số hóa đơn đã đặt: &nbsp;  &nbsp; &nbsp;<input type="text" name="" value="<?php echo number_format(round(floor($row['tong']),-3)).' VNĐ';?>" readonly><br><br>
		<input type="hidden" name="tiendat" value="<?php echo $row['tong'];}?>"><br><br>

		<?php foreach($tongduyet as $row){?>
		Tổng tiền trên số hóa đơn đã duyệt: &nbsp;  &nbsp; &nbsp;<input type="text" name="" value="<?php echo number_format(round(floor($row['tong']),-3)).' VNĐ';?>" readonly><br><br>
		<input type="hidden" name="tienduyet" value="<?php echo $row['tong'];}?>"><br><br>
